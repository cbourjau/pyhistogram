...
class Test_Axis(unittest.TestCase):
    ...
    def test_init_with_contradictory_args(self):
        ...
        self.assertRaises(ValueError, Axis, self.hist, [2, 1])
        self.assertRaises(ValueError, Axis, self.hist, [2, 2])
        # too few edges
        self.assertRaises(ValueError, Axis, self.hist, [2, ])

    def test_number_of_created_bins(self):
        a = Axis(self.hist, [1, 2])
        self.assertEqual(a.nbins, 1)
        a = Axis(self.hist, [1, 2, 5])
        self.assertEqual(a.nbins, 2)

    def test_getter(self):
        a = Axis(self.hist, [1, 2, 3])
